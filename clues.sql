...
SELECT code FROM countries WHERE population = (SELECT MIN(population) FROM countries WHERE region = 'Southern Europe');
...
SELECT countrycode FROM countrylanguages WHERE language = 'Italian' AND percentage = 100.0;
...
SELECT countrycode FROM cities WHERE name LIKE 'Serra%' and NOT countrycode = 'SMR';
...
SELECT cities.name  FROM cities JOIN countries on countries.capital = cities.id WHERE cities.countrycode = 'BRA'
